...
public class RedissonLock extends RedissonExpirable implements RLock {
    ...
    private static final Logger log = LoggerFactory.getLogger(org.redisson.RedissonLock.class);

    private static final ConcurrentMap<String, org.redisson.RedissonLock.ExpirationEntry> expirationRenewalMap = PlatformDependent.newConcurrentHashMap();
...
//        get(lockAsync(leaseTime, unit));
    ...
    private void scheduleExpirationRenewal(final long threadId) {
        ...
        if (expirationRenewalMap.putIfAbsent(getEntryName(), new org.redisson.RedissonLock.ExpirationEntry(threadId, task)) != null) {
            task.cancel();
        }
    }
    ...
    void cancelExpirationRenewal(Long threadId) {
        org.redisson.RedissonLock.ExpirationEntry task = expirationRenewalMap.get(getEntryName());
...
//        return get(tryLockAsync(waitTime, leaseTime, unit));
    ...
    @Override
    public boolean tryLock(long waitTime, long leaseTime, TimeUnit unit, long modifiedThreadId) throws InterruptedException {
        long time = unit.toMillis(waitTime);
        long current = System.currentTimeMillis();
        final long threadId = modifiedThreadId;
        Long ttl = tryAcquire(leaseTime, unit, threadId);
        // lock acquired
        if (ttl == null) {
            return true;
